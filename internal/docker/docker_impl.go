...
// Клиент инициализируется из окружения (DOCKER_HOST/DOCKER_TLS_*) и
// использует Negotiation для совместимости версий API.
...
func (d *dockerClient) PullImage(ctx context.Context, image string, auth AuthConfig) error {
	...
	defer rc.Close()
	// Можно читать поток для логирования: io.Copy(io.Discard, rc)
	return nil
}
...
func (d *dockerClient) InspectContainer(ctx context.Context, id string) (ContainerInspect, error) {
	...
	if inspect.ContainerJSONBase != nil && inspect.ContainerJSONBase.State != nil {
		state = inspect.ContainerJSONBase.State.Status // running/exited/created/...
	}
	image := ""
